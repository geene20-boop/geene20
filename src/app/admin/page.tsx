...
function TabVisibilityCard() {
  ...
  const [localTabs, setLocalTabs] = useState<TabVisibility[]>([]);
  ...
  function toggleVisibility(tab: TabVisibility) {
    setLocalTabs(
      localTabs.map((t) =>
        t.module === tab.module && t.feature === tab.feature
          ? { ...t, visible: t.visible ? 0 : 1 }
          : t
      )
    );
  }
    ...
    <div className="bg-white rounded-xl border p-5 flex flex-col gap-4">
      ...
      <div className="flex gap-2 items-center pt-2 border-t">
        <button
          type="button"
          onClick={saveAll}
          disabled={saving || JSON.stringify(tabs) === JSON.stringify(localTabs)}
