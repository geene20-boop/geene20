...
import { PackingItem, PackingRestock, Worker } from "@/lib/types";
...
export default function PackingRestockPage() {
  ...
  async function toggleLock(row: PackingRestock) {
    if (!enteredBy.trim()) {
      setNameError(true);
      return;
    }
    await apiPut(`/api/packing-restock/${row.id}/lock`, { entered_by: enteredBy, locked: !row.locked });
    await loadRows();
  }

  const canManage = admin.loggedIn || session.isModifier;
  function canDeleteRow(row: PackingRestock) {
