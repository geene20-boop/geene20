...
export async function GET(req: NextRequest) {
  ...
  const rows = db
    .prepare("SELECT * FROM packing_breakage WHERE date BETWEEN ? AND ? ORDER BY date DESC, created_at DESC")
    .all(from, to) as PackingBreakage[];
...
export async function POST(req: NextRequest) {
  ...
  try {
    runInTransaction((db) => {
      ...
      db.prepare(
        "INSERT INTO packing_breakage (id, date, kind, key, qty, worker, entered_by) VALUES (?, ?, ?, ?, ?, ?, ?)"
  ...
  logAudit("packing_breakage", `${body.date} ${packingItemAuditLabel(getDb(), body.key)}`, "create", actor, `-${body.qty}`);
