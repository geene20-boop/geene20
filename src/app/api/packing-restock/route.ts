...
export async function GET(req: NextRequest) {
  ...
  const rows = db
    .prepare("SELECT * FROM packing_restock WHERE date BETWEEN ? AND ? ORDER BY date DESC, created_at DESC")
    .all(from, to) as PackingRestock[];
...
export async function POST(req: NextRequest) {
  ...
  try {
    runInTransaction((db) => {
      ...
      db.prepare(
        "INSERT INTO packing_restock (id, date, kind, key, qty, worker, entered_by) VALUES (?, ?, ?, ?, ?, ?, ?)"
